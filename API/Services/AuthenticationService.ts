...
export default class AuthenticationService {
  public static async onlyAuthenticated(req: Request, res: Response, next: () => void): Promise<any> {
    if (!req.headers.authorization) return res.status(400).json({ success: false, error: "Authentication token not found" });
    ...
    if (bearerToken.exp <= moment().unix()) return res.status(401).json({ success: false, error: "Your token has expired!" });
